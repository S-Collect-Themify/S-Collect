...
export function useReturnRequests() {
  ...
  const { data: refundsData, error: refundsError } = useQuery({
    queryKey: ['vendor-refunds'],
    queryFn: () => getRefunds({ limit: 100 }),
    refetchOnWindowFocus: false,
    retry: 1,
  });

  // Fetch sub-orders for additional returns context
  ...
  const allItems: ReturnItem[] = useMemo(() => {
    ...
    if (refundsData?.items && Array.isArray(refundsData.items)) {
      refundsData.items.forEach((rf) => {
        const firstItem = rf.items?.[0] || ({} as any);
        const dateObj = new Date(rf.createdAt);
        ...
        itemsList.push({
          ...
          orderId: `#ORD-${rf.orderId.slice(0, 8).toUpperCase()}`,
          customerName: custName,
          customerEmail: rf.customer?.email || 'customer@example.com',
          customerPhone: rf.customer?.phoneNumber || '',
          ...
          rawId: rf.id,
          rawStatus: rf.status,
          uploadedImages: rf.imageUrls || [],
        });
      });
    }

    // 2. Process sub-orders if refunds didn't cover all return requests
    if (subOrdersData?.items && Array.isArray(subOrdersData.items)) {
      subOrdersData.items.forEach((sub, idx) => {
        // Skip if already included via refunds
        if (itemsList.some((existing) => existing.rawId === sub.id)) {
          return;
        }
        ...
        itemsList.push({
          ...
          productImage,
          reason: firstProduct.reason || sub.statusOverrideReason || '',
          requestedDate: formattedDate,
          status: mappedStatus,
