...
export async function exportToXLSX<T extends Record<string, any>>(
  ...
  const downloadFilename = baseName.includes(dateStr)
    ? `${baseName}.xlsx`
...
/**
 * Export table data as PDF using responsive printable window document
 */
export function exportToPDF<T extends Record<string, any>>(
  ...
  const tableHeaders = headers
    .map(
      (h) =>
        `<th style="padding: 10px; border-bottom: 2px solid #e5e7eb; text-align: start; font-size: 12px; color: #6b7280;">${h.label}</th>`
    )
    .join('');
    ...
    .map(
      (item) =>
        `<tr style="border-bottom: 1px solid #f3f4f6;">${headers
          .map(
            (h) =>
              `<td style="padding: 10px; font-size: 12px; color: #111827;">${String(
                item[h.key as keyof T] ?? ''
              )}</td>`
          )
          .join('')}</tr>`
    )
