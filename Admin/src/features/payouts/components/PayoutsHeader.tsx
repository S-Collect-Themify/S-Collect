...
export default function PayoutsHeader({ onExportExcel, onExportPDF }: PayoutsHeaderProps) {
          ...
          <PortalDropdown
            ...
            trigger={({ toggle }) => (
              <button
                type="button"
                onClick={toggle}
                className="inline-flex items-center gap-2 px-3.5 sm:px-4 py-2 sm:py-2.5 bg-gray-950 text-white text-xs sm:text-label-md font-semibold rounded-xl hover:bg-gray-800 transition-all active:scale-95 cursor-pointer shadow-sm shrink-0"
              >
                <Download size={16} />
                <span>{t('payouts.export', 'Export')}</span>
              </button>
            )}
          >
            {({ close }) => (
              <div>
                ...
                >
                  ...
                  <span>{t('payouts.exportExcel', 'Export as Excel (.csv)')}</span>
                </button>
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  ...
                  <span>{t('payouts.exportPdf', 'Export as PDF (.pdf)')}</span>
                </button>
              </div>
            )}
          </PortalDropdown>
