...
interface AttributeValueModalProps {
  isOpen: boolean;
  onClose: () => void;
  attribute: StandardAttribute | null;
  value?: StandardAttributeValue | null;
  onSubmit: (
...
export const AttributeValueModal = ({
  ...
  const handleFormSubmit = async (values: ValueFormValues) => {
    if (isEdit) {
      const updateDto: UpdateStandardAttributeValueDto = {
      ...
      const createDto: CreateStandardAttributeValueDto = {
        value: values.value.trim(),
        valueAr: values.valueAr.trim(),
        sortOrder: Number(values.sortOrder) || 0,
      };
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="p-6 space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl bg-gray-900 hover:bg-gray-800 text-white text-sm font-semibold transition-colors cursor-pointer shadow-xs active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed min-w-25"
            >
              {isSubmitting ? (
                <>
