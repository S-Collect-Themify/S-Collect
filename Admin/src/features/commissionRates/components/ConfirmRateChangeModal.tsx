...
export default function ConfirmRateChangeModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in-up">
      <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-[400px] shadow-2xl border border-gray-100 text-center space-y-5">
        {/* Warning Icon Circle */}
        ...
        <div className="flex flex-col md:flex-row items-center gap-3 pt-2">
          <button
            type="button"
            onClick={onClose}
            className="w-full md:w-1/2 h-11 border border-gray-200 rounded-xl text-sm font-bold text-gray-800 hover:bg-gray-50 transition-all cursor-pointer order-2 md:order-1 flex items-center justify-center"
          >
          ...
          <button
            type="button"
            onClick={onConfirm}
            className="w-full md:w-1/2 h-11 bg-black text-white hover:bg-gray-800 rounded-xl text-sm font-bold shadow-2xs transition-all active:scale-95 cursor-pointer order-1 md:order-2 flex items-center justify-center"
          >
