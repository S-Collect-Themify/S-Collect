...
import type { BulkDiscountFormData } from '../types';
          ...
          <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                <option value="FIXED">{isAr ? 'ثابت (مبلغ)' : 'Fixed Amount'}</option>
              </select>
            </div>
              ...
              <div className="relative">
                <input
                  ...
                  {...register('discountValue', {
                    required: isAr ? 'هذا الحقل مطلوب' : 'Discount value is required',
                    min: {
                      value: 0.01,
                      message: isAr ? 'يجب أن تكون القيمة أكبر من 0' : 'Value must be greater than 0',
                    },
                  })}
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1.5">
                {isAr ? 'تاريخ انتهاء الخصم (اختياري)' : 'Discount Expiry Date (Optional)'}
              </label>
              <input
                type="date"
                {...register('expiryDate')}
              ...
              >
                {isPending ? (
                  <>
                    ...
                    <span>{isAr ? 'جاري الحفظ...' : 'Applying...'}</span>
                  </>
                ) : (
                  <span>{isAr ? 'تطبيق الخصم' : 'Apply Discount'}</span>
                )}
              </button>
            </div>
          </form>
