    ...
    <div className="flex flex-col min-h-screen bg-gray-100 pb-20">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 z-10">
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="w-full rounded-xl bg-gray-900 py-3.5 text-sm font-semibold text-white transition hover:bg-gray-800 disabled:opacity-50 active:scale-[0.98] cursor-pointer"
        >
          {isSaving ? '...' : t('inventoryPage.saveChanges', 'Save Changes')}
        </button>
      </div>
    </div>
  );
